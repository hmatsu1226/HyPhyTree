...
optpara <- optim(tmp, myfn_H2, method="L-BFGS-B", lower=0, upper=2*max(D, na.rm = TRUE), control=list(trace=1, maxit=2000))
print(optpara$value)
for(i in 1:length(na_pairs)){
...
write.table(D2, paste("out_imputation/optD_L",L,"_H2.txt",sep=""), sep="\t", col.names=F, row.names=F)
